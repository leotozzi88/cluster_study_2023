...
#### Significance of clustering indexes (in line with Dinga et al.) #### 
nclu_test=c(2:15)
n_sim_rep=10000
...
# Test which features don't differ across clusters
ps=c()
for (feat in img_vars_names_final){
  res=anova(aov(data=data, data[, feat]~clu))
  ps=append(ps, res$`Pr(>F)`[1])
}
img_vars_names_final[ps>0.05]
