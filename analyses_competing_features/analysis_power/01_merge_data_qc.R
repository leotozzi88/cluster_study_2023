...
img=vroom('appeal/analyses_competing_features/analysis_power/powers_tab.csv')
img <- img %>% rename_all(~sub("Var", "power_", .))
...
for (sub in img$id){
  ...
  if (fd[fd$id==sub,'fd03_rest']>0.25 & !is.na(fd[fd$id==sub,'fd03_rest'])){
    img[img$id==sub,grep("power", names(img), value=TRUE)]=NA
  }
  }
}

#write.csv(img, 'appeal/analyses_competing_features/analysis_power/circ_table_fdremoved.csv', row.names = FALSE)

# Remove connections with bad coverage
img_vars=grep("power", names(img), value = TRUE)
img_vars_bad=c()
for (feat in img_vars){
  if (sum(is.na(img[, feat]))>0.10*nrow(img)){
    img_vars_bad=append(img_vars_bad, feat)
  }
}
img_vars=img_vars[!(img_vars %in% img_vars_bad)]

# Keep only subjects with the features of interest
img_nona=na.omit(img[,c('id', img_vars) ])

# Merge imaging and questionnaires
merged=merge(ques, img_nona, by = 'id')

# Merge with behavior
merged=merge(merged, beh, by = 'id', all.x = TRUE)

# Save data
write.csv(merged, 'appeal/analyses_competing_features/analysis_power/dataset_merged.csv', row.names = FALSE)

# Keep only subjects who passed QC
merged_qc=merged[merged$id %in% subs_qc, ]

# Keep only subjects with imaging
img_vars=grep("power", names(merged_qc), value = TRUE)
...
# Save data
write.csv(merged_qc, 'appeal/analyses_competing_features/analysis_power/dataset_merged_qc.csv', row.names = FALSE)
