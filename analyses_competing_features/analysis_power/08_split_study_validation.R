...
set.seed(123123)
data=as.data.frame(vroom('analyses_competing_features/analysis_power/out/dataset_merged_qc_imputed_combat_clin_std_clu_sympbeh_spl.csv'))
...
for (study in unique(data$study_reduced)){
  ...
  write.csv(symps_results_merged, paste('analyses_competing_features/analysis_power/out/symps_results_study_merged', study, '.csv', sep=''), row.names = F)
  ...
  write.csv(beh_results_merged, paste('analyses_competing_features/analysis_power/out/beh_results_study_merged', study, '.csv', sep=''), row.names = F)
