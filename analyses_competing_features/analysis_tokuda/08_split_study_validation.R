...
for (study in unique(data$study_reduced)){
  ...
  clu_mean_mat=matrix(nrow = 6, ncol = length(img_vars))
  for (clu in 1:6){
...
for (study in unique(data$study_reduced)){
  ...
  write.csv(symps_results_merged, paste('appeal/analyses_competing_features/analysis_tokuda/symps_results_merged_ho_', study, '.csv', sep=''), row.names = F)
...
for (study in unique(data$study_reduced)){
  ...
  write.csv(beh_results_merged, paste('appeal/analyses_competing_features/analysis_tokuda/beh_results_merged_ho_', study, '.csv', sep=''), row.names = F)
