...
set.seed(123123)
data=read.csv('analyses_competing_features/analysis_tokuda/out/dataset_merged_qc_imputed_combat_clin_std_clu_sympbeh_spl.csv')
...
data=splitstudy_and_cluster(data, nclu = 3, img_vars_names = img_vars_names, studyvec = data$study_reduced)
...
for (study in unique(data$study_reduced)){
  ...
  write.csv(symps_results_merged, paste('analyses_competing_features/analysis_tokuda/out/symps_results_study_merged', study, '.csv', sep=''), row.names = F)
  ...
  write.csv(beh_results_merged, paste('analyses_competing_features/analysis_tokuda/out/beh_results_study_merged', study, '.csv', sep=''), row.names = F)
