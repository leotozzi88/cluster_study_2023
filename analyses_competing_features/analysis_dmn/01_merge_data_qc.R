...
img=read.csv('appeal/analyses_competing_features/analysis_dmn/dmn_tab.csv')
img <- img %>% rename_all(~sub("Var", "dmn_", .))
...
#write.csv(img, 'appeal/analyses_competing_features/analysis_rsonly/circ_table_fdremoved.csv', row.names = FALSE)

# Remove connections with bad coverage
img_vars=grep("dmn_", names(img), value = TRUE)
img_vars_bad=c()
...
# Save data
write.csv(merged, 'appeal/analyses_competing_features/analysis_dmn/dataset_merged.csv', row.names = FALSE)

# Keep only subjects who passed QC
merged_qc=merged[merged$id %in% subs_qc, ]

# Keep only subjects with imaging
img_vars=grep("dmn_", names(merged_qc), value = TRUE)
...
# Save data
write.csv(merged_qc, 'appeal/analyses_competing_features/analysis_dmn/dataset_merged_qc.csv', row.names = FALSE)
