...
set.seed(123123)
data=read.csv('analyses_competing_features/analysis_dmn/out/dataset_merged_qc_imputed_combat_clin_std_clu_sympbeh.csv')

#### Validation of clusters in split-half data ####

img_vars_names=grep("dmn_", names(data), value = TRUE)

#### Split half assignment ####

splits=splithalf_and_cluster(data, nclu = 2, img_vars_names = img_vars_names)
...
write.csv(symps_results_merged, 'analyses_competing_features/analysis_dmn/out/symps_results_sh_merged.csv', row.names = F)
...
write.csv(beh_results_merged, 'analyses_competing_features/analysis_dmn/out/beh_results_sh_merged.csv', row.names = F)

#### Save data ####

write.csv(data, 'analyses_competing_features/analysis_dmn/out/dataset_merged_qc_imputed_combat_clin_std_clu_sympbeh_spl.csv')
